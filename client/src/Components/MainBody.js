...
import { incrementPage, decrementPage, fetchImages } from '../redux/dataSlice.js'
...
const MainBody = () => {
    ...
    const handleNext = () => {
        ...
        if (storedIndex === currentImgs.length - 2) {
            dispatch(fetchImages({category:storedCategory, page:storedIndex+1, numPerPage: 9}))
        }
    ...
    return (
        <div className='w-full h-full flex flex-col items-center justify-center' id='MainBody'>
            <div className='flex items-center gap-20'>
                <Button text='Prev' pageNumber={storedIndex+1} action={decrementPage()}></Button>
